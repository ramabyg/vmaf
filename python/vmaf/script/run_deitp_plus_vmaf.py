...
from vmaf.core.quality_runner import VmafQualityRunner, DlbDeitpVmafQualityRunner
...
def main():
    ...
    #Set optional_dict with deitp parameters
    eotf = get_cmd_option(sys.argv, 6, len(sys.argv), '--eotf')
    clrFmt = get_cmd_option(sys.argv, 6, len(sys.argv), '--clrFmt')
    gamma = get_cmd_option(sys.argv, 6, len(sys.argv), '--gamma')
    minLum = get_cmd_option(sys.argv, 6, len(sys.argv), '--minLum')
    maxLum = get_cmd_option(sys.argv, 6, len(sys.argv), '--maxLum')
    Yuv2RgbExt = get_cmd_option(sys.argv, 6, len(sys.argv), '--Yuv2RgbExt')
    YuvXferSpec = get_cmd_option(sys.argv, 6, len(sys.argv), '--YuvXferSpec')
    Rng = get_cmd_option(sys.argv, 6, len(sys.argv), '--Rng')
    RgbDef = get_cmd_option(sys.argv, 6, len(sys.argv), '--RgbDef')


    if optional_dict is None:
        optional_dict = {}
    if eotf:
        optional_dict['eotf'] = eotf
    if clrFmt:
        optional_dict['clrFmt'] = clrFmt
    if gamma:
        optional_dict['gamma'] = gamma
    if minLum:
        optional_dict['minLum'] = minLum
    if maxLum:
        optional_dict['maxLum'] = maxLum
    if Yuv2RgbExt:
        optional_dict['Yuv2RgbExt'] = Yuv2RgbExt
    if YuvXferSpec:
        optional_dict['YuvXferSpec'] = YuvXferSpec
    if Rng:
        optional_dict['Rng'] = Rng
    if RgbDef:
        optional_dict['RgbDef'] = RgbDef
    #make sure we are not sending empty dictionary
